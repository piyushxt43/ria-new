...
export default function FeaturesSection() {
  return (
    <section className="py-16 md:py-20 bg-gradient-to-b from-white to-indigo-50/40" id="coach">
        ...
        {features.map((feature, index) => (
          ...
          >
            ...
            <div className="w-full md:w-1/2 text-center md:text-left">
              <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-[0.3em] bg-white text-indigo-600 ring-1 ring-indigo-100 mb-4">
                {feature.badge}
              </span>
              <h3 className="text-2xl md:text-3xl lg:text-4xl font-bold text-foreground mb-4">
                {feature.title}
              </h3>
              <p className="text-lg md:text-xl text-muted-foreground">{feature.description}</p>
            </div>
          </div>
        ))}
      </div>
